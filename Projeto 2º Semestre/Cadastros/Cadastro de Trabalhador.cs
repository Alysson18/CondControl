    ...
    public partial class Cadastro_de_Trabalhador : Form
    {
        ...
        private void btLiberar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                MySqlCommand dbcomand = new MySqlCommand("insert into tb_controleacesso (Nome, Telefone, RG, Endereco, Apartamento, Proprietario, Data, Hora_de_Entrada, Porteiro_Responsavel_Entrada, Alteracao, Codigo_de_Acesso)" +
                    " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", con.conn);
                ...
                dbcomand.Parameters.Add("@Apartamento", MySqlDbType.VarChar, 45).Value = Trabalhador.Casa;
                dbcomand.Parameters.Add("@Proprietario", MySqlDbType.VarChar, 45).Value = Trabalhador.Propietario;
                dbcomand.Parameters.Add("@Data", MySqlDbType.VarChar, 45).Value = DateTime.Now.ToShortDateString();
                dbcomand.Parameters.Add("@Hora_de_Entrada", MySqlDbType.VarChar, 45).Value = DateTime.Now.ToLongTimeString();
                dbcomand.Parameters.Add("@Porteiro_Responsavel_Entrada", MySqlDbType.VarChar, 45).Value = Informacoes.Nome;
                dbcomand.Parameters.Add("@Alteracao", MySqlDbType.VarChar, 45).Value = 0;
        ...
        private void btEncerrar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MySqlCommand dbcomand = new MySqlCommand("Update tb_controleacesso set Hora_de_Saida = ?, Porteiro_Responsavel_Saida = ? where Codigo_de_Acesso = ? And Data = ?", con.conn);

                dbcomand.Parameters.Add("@Hora_de_Saida", MySqlDbType.VarChar, 45).Value = DateTime.Now.ToLongTimeString();
